...
if __name__ == "__main__":
    ...
    rtsne = ReversibleTSNE(n_components=3, explained_variance_for_dimensionality_reduction=99,
                           approximate=False,
                           method="barnes_hut", random_state=42)

    rtsne.fit(X_train_noisy)
    ...
    print(X_test_noisy.shape, rtsne.fit_transform(X_test_noisy).shape)
    X_reconstructed_rtsne = rtsne.inverse_transform(rtsne.fit_transform(X_test_noisy), X_test_noisy)
    # %%
    plot_digits(X_test, "Uncorrupted test images")
    plot_digits(
        X_reconstructed_pca,
    ...
    plot_digits(
        X_reconstructed_rtsne,
        (
            "Reversible t-SNE reconstruction\n"
            f"MSE: {np.mean((X_test - X_reconstructed_rtsne) ** 2):.2f}"
        ),
    )
    # %%
